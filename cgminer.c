/*
 * Copyright 2013-2014 Juraj Puchký - Devtech <sjurajpuchky@seznam.cz>
 ...
 *
 * Notice:
 * * upcomming from previous version
 * - added support immediate status
 * - added ignore of SIGFPE, hate them, they crash me too much
 */
...
struct strategies strategies[] = { { "Failover" }, { "Round Robin" },
		{ "Rotate" }, { "Load Balance" }, { "Balance" }, };

// Bitops
unsigned char *usedBlockMap;
...
struct sigaction termhandler, inthandler, fpehandler;
...
void get_datestamp(char *f, struct timeval *tv) {
	...
	const time_t tmp_time = tv->tv_sec;
	tm = localtime(&tmp_time);
	sprintf(f, "[%d-%02d-%02d %02d:%02d:%02d]", tm->tm_year + 1900,
 ...
 * stratum+tcp or by detecting a stratum server response */bool detect_stratum(
		struct pool *pool, char *url) {
...
static struct opt_table opt_config_table[] =
		{
				...
				OPT_WITH_ARG("--monitor|-m",
						...
						opt_hidden), OPT_WITHOUT_ARG("--no-restart",
				...
				OPT_WITH_ARG("--retries",
						set_null, NULL, NULL,
						opt_hidden), OPT_WITH_ARG("--retry-pause",
						set_null, NULL, NULL,
						opt_hidden),
						OPT_WITH_ARG("--rotate",
				...
				OPT_WITH_ARG("--sharelog",
						...
						"Append share log to file"), OPT_WITH_ARG("--shares",
				...
				OPT_WITH_ARG("--pools",
						opt_set_bool, NULL, NULL, opt_hidden), OPT_ENDTABLE };
			...
			if ((opt->type & OPT_HASARG) && json_is_string(val)) {
				...
				for (n = 0; n < size && !err; n++) {
					if (json_is_string(json_array_get(val, n)))
						err = opt->cb_arg(
								json_string_value(json_array_get(val, n)),
								opt->u.arg);
...
static char *load_config(const char *arg, void __maybe_unused *unused) {
	...
	if (!json_is_object(config)) {
		json_error = malloc(
				JSON_LOAD_ERROR_LEN + strlen(arg) + strlen(err.text));
		if (!json_error)
...
static struct opt_table opt_cmdline_table[] = { OPT_WITH_ARG("--config|-c",
		...
		OPT_WITHOUT_ARG("--version|-V",
				...
				"Display version and exit"), OPT_ENDTABLE };
...
static void __build_gbt_coinbase(struct pool *pool) {
	...
	*extra_len += 4;
	hex2bin(coinbase + 42 + *extra_len, pool->coinbasetxn + 84 + (orig_len * 2),
			cbt_len - orig_len - 42);
	pool->nonce2++;
...
static void update_gbt(struct pool *pool) {
	...
	if (val) {
		...
		if (rc) {
			applog(LOG_DEBUG,
					"Successfully retrieved and updated GBT from pool %u %s",
					pool->pool_no, pool->rpc_url);
			cgtime(&pool->tv_idle);
		} else {
			applog(LOG_DEBUG,
					"Successfully retrieved but FAILED to decipher GBT from pool %u %s",
					pool->pool_no, pool->rpc_url);
		}
		json_decref(val);
		free_work(work);
	} else {
		applog(LOG_DEBUG, "FAILED to update GBT from pool %u %s", pool->pool_no,
				pool->rpc_url);
	}
...
static bool gbt_decode(struct pool *pool, json_t *res_val) {
	...
	previousblockhash = json_string_value(
			json_object_get(res_val, "previousblockhash"));
	...
	if (!previousblockhash || !target || !coinbasetxn || !longpollid || !expires
			|| !version || !curtime || !bits) {
...
#ifdef HAVE_CURSES
extern uint32_t supradrive_total;
...
static void curses_print_status(void) {
	...
	mvwprintw(statuswin, 0, 0, " " PACKAGE " version " VERSION " - Started: %s",
			datestamp);
#ifdef WANT_CPUMINE
	if (opt_n_threads)
	...
	wclrtoeol(statuswin);
	mvwprintw(statuswin, 5, 0,
			" Block: %s...  Diff:%s  Started: %s  Best share: %s   ",
			current_hash, block_diff, blocktime, best_share);
	supradrive_totals = supradrive_total - supradrive_totals_old;
	supradrive_totals_old = supradrive_total;
	mvwprintw(statuswin, 6, 0, " CPU up:    %c%08x %c%08x %c%08x %c%08x %c%08x %c%08x",
			supradrive_currentupnonce[0] != supradrive_currentupnonce[1] ? '*' : '=',
			supradrive_currentupnonce[0],
			supradrive_currentupnonce[1] != supradrive_currentupnonce[2] ? '*' : '=',
			supradrive_currentupnonce[1],
			supradrive_currentupnonce[2] != supradrive_currentupnonce[3] ? '*' : '=',
			supradrive_currentupnonce[2],
			supradrive_currentupnonce[3] != supradrive_currentupnonce[4] ? '*' : '=',
			supradrive_currentupnonce[3],
			supradrive_currentupnonce[4] != supradrive_currentupnonce[5] ? '*' : '=',
			supradrive_currentupnonce[4],
			supradrive_currentupnonce[0] != supradrive_currentupnonce[5] ? '*' : '=',
			supradrive_currentupnonce[5]
	);
	wclrtoeol(statuswin);
	mvwprintw(statuswin, 7, 0, " CPU down:  %c%08x %c%08x %c%08x %c%08x %c%08x %c%08x",
			supradrive_currentdownnonce[0] != supradrive_currentdownnonce[1] ? '*' : '=',
			supradrive_currentdownnonce[0],
			supradrive_currentdownnonce[1] != supradrive_currentdownnonce[2] ? '*' : '=',
			supradrive_currentdownnonce[1],
			supradrive_currentdownnonce[2] != supradrive_currentdownnonce[3] ? '*' : '=',
			supradrive_currentdownnonce[2],
			supradrive_currentdownnonce[3] != supradrive_currentdownnonce[4] ? '*' : '=',
			supradrive_currentdownnonce[3],
			supradrive_currentdownnonce[4] != supradrive_currentdownnonce[5] ? '*' : '=',
			supradrive_currentdownnonce[4],
			supradrive_currentdownnonce[0] != supradrive_currentdownnonce[5] ? '*' : '=',
			supradrive_currentdownnonce[5]
    );
	wclrtoeol(statuswin);
	mvwprintw(statuswin, 8 , 0, " nonce count: %d%s semi results %d nonce %d",
			supradrive_totals > 1024 ? supradrive_totals / 1024 : supradrive_totals,
			supradrive_totals > 1024 ? "kH/s" : "H/s",
			semiResults,
			supradrive_total
    );
	wclrtoeol(statuswin);
	...
	mvwprintw(statuswin, devcursor + 1, 1,
...
static void curses_print_devstatus(int thr_id) {
	...
	if (cgpu->cgminer_id >= start_devices
			|| devcursor + cgpu->cgminer_id > LINES - 2)
		return;

	cgpu->utility = cgpu->accepted / total_secs * 60;

	wmove(statuswin, devcursor + cgpu->cgminer_id, 0);
	wprintw(statuswin, " %s %*d: ", cgpu->drv->name, dev_width,
			cgpu->device_id);
	logline[0] = '\0';
	...
	wprintw(statuswin, "/%6sh/s | A:%*d R:%*d HW:%*d U:%*.2f/m",
			displayed_hashes, awidth, cgpu->accepted, rwidth, cgpu->rejected,
			hwwidth, cgpu->hw_errors, uwidth + 3, cgpu->utility);
...
static void share_result(json_t *val, json_t *res, json_t *err,
	...
	if (json_is_true(res) || (work->gbt && json_is_null(res))) {
		...
		if (!QUIET) {
			if (total_pools > 1)
				applog(LOG_NOTICE, "Accepted %s %s %d pool %d %s%s", hashshow,
						cgpu->drv->name, cgpu->device_id, work->pool->pool_no,
						resubmit ? "(resubmit)" : "", worktime);
			else
				applog(LOG_NOTICE, "Accepted %s %s %d %s%s", hashshow,
...
static bool submit_upstream_work(struct work *work, CURL *curl, bool resubmit) {
	...
	val = json_rpc_call(curl, pool->rpc_url, pool->rpc_userpass, s, false,
			false, &rolltime, pool, true);
	...
	if (!QUIET) {
		...
		if (opt_worktime) {
			...
			sprintf(worktime,
					...
					(unsigned long) swab32(
							*(uint32_t *) &(work->data[opt_scrypt ? 32 : 28])),
					(unsigned long) swab32(
							*(uint32_t *) &(work->data[opt_scrypt ? 28 : 24])),
...
static double DIFFEXACTONE =
		26959946667150639794667015087019630673637144422540572481103610249215.0;
...
static bool get_upstream_work(struct work *work, CURL *curl) {
	...
	pool_stats->getwork_wait_rolling += ((double) tv_elapsed.tv_sec
...
static void sigfpehandler(int __maybe_unused sig) {
...
static bool clone_available(void) {
	...
	out_unlock: mutex_unlock(stgd_lock);
...
static void pool_died(struct pool *pool) {
	if (!pool_tset(pool, &pool->idle)) {
		cgtime(&pool->tv_idle);
		if (pool == current_pool()) {
			applog(LOG_WARNING, "Pool %d %s not responding!", pool->pool_no,
					pool->rpc_url);
			switch_pools(NULL );
		} else
			applog(LOG_INFO, "Pool %d %s failed to return work", pool->pool_no,
					pool->rpc_url);
	}
}
...
static void *submit_work_thread(void *userdata) {
	...
	if (work->stratum) {
		...
		sprintf(s,
				...
				pool->rpc_user, work->job_id, work->nonce2, work->ntime,
				noncehex, sshare->id);
		free(noncehex);
		...
		while (time(NULL ) < sshare->sshare_time + 120) {
			...
			if (!pool_tset(pool, &pool->submit_fail) && cnx_needed(pool)) {
				applog(LOG_WARNING, "Pool %d stratum share submission failure",
						pool->pool_no);
				total_ro++;
				pool->remotefail_occasions++;
			}
	...
	while (!submit_upstream_work(work, ce->curl, resubmit)) {
		...
		if (stale_work(work, true)) {
			applog(LOG_NOTICE,
					"Pool %d share became stale while retrying submit, discarding",
					pool->pool_no);
...
void switch_pools(struct pool *selected) {
	...
	if (pool != last_pool && pool_strategy != POOL_LOADBALANCE
			&& pool_strategy != POOL_BALANCE) {
		applog(LOG_WARNING, "Switching to pool %d %s", pool->pool_no,
				pool->rpc_url);
...
static void stage_work(struct work *work) {
	applog(LOG_DEBUG, "Pushing work from pool %d to hash queue",
			work->pool->pool_no);
	work->work_block = work_block;
	test_work_current(work);
	hash_push(work);
...
static void display_pool_summary(struct pool *pool) {
	...
	if (curses_active_locked()) {
		...
		efficiency =
				pool->getwork_requested ?
						pool->accepted * 100.0 / pool->getwork_requested : 0.0;
...
void write_config(FILE *fcfg) {
	...
	for (i = 0; i < total_pools; i++) {
		fprintf(fcfg, "%s\n\t{\n\t\t\"url\" : \"%s%s%s%s\",", i > 0 ? "," : "",
				pools[i]->rpc_proxy ?
						json_escape(
								(char *) proxytype(pools[i]->rpc_proxytype)) :
						"",
				pools[i]->rpc_proxy ? json_escape(pools[i]->rpc_proxy) : "",
				pools[i]->rpc_proxy ? "|" : "", json_escape(pools[i]->rpc_url));
		fprintf(fcfg, "\n\t\t\"user\" : \"%s\",",
				json_escape(pools[i]->rpc_user));
		fprintf(fcfg, "\n\t\t\"pass\" : \"%s\"\n\t}",
	...
	if (nDevs) {
		...
		fputs(",\n\"intensity\" : \"", fcfg);
		for (i = 0; i < nDevs; i++)
			fprintf(fcfg, gpus[i].dynamic ? "%sd" : "%s%d", i > 0 ? "," : "",
					gpus[i].intensity);
		fputs("\",\n\"vectors\" : \"", fcfg);
		for (i = 0; i < nDevs; i++)
	...
	if (schedstart.enable)
		fprintf(fcfg, ",\n\"sched-time\" : \"%d:%d\"", schedstart.tm.tm_hour,
				schedstart.tm.tm_min);
	if (schedstop.enable)
		fprintf(fcfg, ",\n\"stop-time\" : \"%d:%d\"", schedstop.tm.tm_hour,
				schedstop.tm.tm_min);
	if (opt_socks_proxy && *opt_socks_proxy)
		fprintf(fcfg, ",\n\"socks-proxy\" : \"%s\"",
				json_escape(opt_socks_proxy));
#ifdef HAVE_OPENCL
	for (i = 0; i < nDevs; i++)
	...
	if (strcmp(opt_api_description, PACKAGE_STRING) != 0)
		fprintf(fcfg, ",\n\"api-description\" : \"%s\"",
				json_escape(opt_api_description));
	if (opt_api_groups)
		fprintf(fcfg, ",\n\"api-groups\" : \"%s\"",
				json_escape(opt_api_groups));
	if (opt_icarus_options)
		fprintf(fcfg, ",\n\"icarus-options\" : \"%s\"",
				json_escape(opt_icarus_options));
	if (opt_icarus_timing)
		fprintf(fcfg, ",\n\"icarus-timing\" : \"%s\"",
				json_escape(opt_icarus_timing));
#ifdef USE_USBUTILS
	if (opt_usb_select)
...
static void display_pools(void) {
	...
	immedok(logwin, true);
	clear_logwin();
	updated: for (i = 0; i < total_pools; i++) {
		pool = pools[i];
		...
		wlogprint("%s Priority %d: %s  User:%s\n",
				pool->idle ? "Dead" : "Alive", pool->prio, pool->rpc_url,
				pool->rpc_user);
...
static void display_options(void) {
	...
	wlogprint("[D]ebug:%s\n[P]er-device:%s\n[Q]uiet:%s\n[V]erbose:%s\n"
			...
			opt_log_output ? "on" : "off", opt_protocol ? "on" : "off",
			opt_worktime ? "on" : "off", opt_compact ? "on" : "off",
			opt_log_interval);
...
static void set_options(void) {
	...
	immedok(logwin, true);
	clear_logwin();
	retry: wlogprint("[Q]ueue: %d\n[S]cantime: %d\n[E]xpiry: %d\n"
			"[W]rite config file\n[C]gminer restart\n", opt_queue, opt_scantime,
			opt_expiry);
	wlogprint("Select an option or any other key to return\n");
	input = getch();
	...
	if (!strncasecmp(&input, "q", 1)) {
		...
		sprintf(prompt, "Config filename to write (Enter for default) [%s]",
				filename);
...
static void hashmeter(int thr_id, struct timeval *diff, uint64_t hashes_done) {
	...
	local_secs = (double) total_diff.tv_sec
	...
	sprintf(statusline,
			"%s(%ds):%s (avg):%sh/s | A:%d  R:%d  HW:%d  U:%.1f/m  WU:%.1f/m",
			want_per_device_stats ? "ALL " : "", opt_log_interval,
			displayed_rolling, displayed_hashes, total_accepted, total_rejected,
			hw_errors, utility, total_diff1 / total_secs * 60);

	local_mhashes_done = 0;
	out_unlock: mutex_unlock(&hash_lock);
...
static bool parse_stratum_response(struct pool *pool, char *s) {
	...
	if (!sshare) {
		if (json_is_true(res_val))
			applog(LOG_NOTICE, "Accepted untracked stratum share from pool %d",
					pool->pool_no);
		else
			applog(LOG_NOTICE, "Rejected untracked stratum share from pool %d",
					pool->pool_no);
		goto out;
	}
...
void clear_stratum_shares(struct pool *pool) {
	...
	if (cleared) {
		applog(LOG_WARNING,
				"Lost %d shares due to stratum disconnect on pool %d", cleared,
				pool->pool_no);
...
static void stratum_resumed(struct pool *pool) {
	...
	if (pool_tclear(pool, &pool->idle)) {
		applog(LOG_INFO, "Stratum connection to pool %d resumed",
				pool->pool_no);
		pool_resus(pool);
	}
}
...
static void *stratum_thread(void *userdata) {
	...
	while (42) {
		 ...
		 * as dead */
		if (!sock_full(pool)
				&& (sel_ret = select(pool->sock + 1, &rd, NULL, NULL, &timeout))
						< 1) {
			applog(LOG_DEBUG, "Stratum select failed on pool %d with value %d",
					pool->pool_no, sel_ret);
			s = NULL;
		} else
			s = recv_line(pool);
		if (!s) {
			applog(LOG_NOTICE, "Stratum connection to pool %d interrupted",
					pool->pool_no);
			pool->getfail_occasions++;
			total_go++;
...
static bool stratum_works(struct pool *pool) {
	applog(LOG_INFO, "Testing pool %d stratum %s", pool->pool_no,
			pool->stratum_url);
...
static bool pool_active(struct pool *pool, bool pinging) {
	...
	if (pool->has_gbt)
		applog(LOG_DEBUG, "Retrieving block template from pool %s",
				pool->rpc_url);
	else
	...
	if (!pool->probed && !opt_fix_protocol) {
		...
		val = json_rpc_call(curl, pool->rpc_url, pool->rpc_userpass, gbt_req,
				true, false, &rolltime, pool, false);
		if (val) {
	...
	if (pool->stratum_url && !opt_fix_protocol && stratum_works(pool)) {
		applog(LOG_NOTICE, "Switching pool %d %s to %s", pool->pool_no,
	...
	if (val) {
		...
		if (pool->hdr_path) {
			...
			if (strstr(hdr_path, "://")) {
				...
				pool->lp_url = malloc(
						strlen(pool->rpc_url) + strlen(copy_start) + 2);
				if (!pool->lp_url) {
					applog(LOG_ERR, "Malloc failure in pool_active");
					return false;
				}

				sprintf(pool->lp_url, "%s%s%s", pool->rpc_url,
...
static void gen_stratum_work(struct pool *pool, struct work *work) {
	...
	hex2bin(coinbase + pool->swork.cb1_len + pool->n1_len, work->nonce2,
			pool->n2size);
	hex2bin(coinbase + pool->swork.cb1_len + pool->n1_len + pool->n2size,
			pool->swork.coinbase2, pool->swork.cb2_len);
	...
	sprintf(header, "%s%s%s%s%s%s%s", pool->swork.bbversion,
			pool->swork.prev_hash, merkle_hash, pool->swork.ntime,
			pool->swork.nbit, "00000000", /* nonce */
			workpadding);
	...
	applog(LOG_DEBUG, "Work job_id %s nonce2 %s ntime %s", work->job_id,
...
static void hash_sole_work(struct thr_info *mythr) {
	...
	struct timeval getwork_start, tv_start, *tv_end, tv_workstart,
			tv_lastupdate;
	...
	while (42) {
		...
		do {
			...
			if (unlikely((long)sdiff.tv_sec < cycle)) {
				...
				max_nonce = max_nonce * 0x400
						/ (((cycle * 1000000) + sdiff.tv_usec)
								/ (cycle * 1000000 / 0x400));
 ...
 * while still in use. */
static void fill_queue(struct thr_info *mythr, struct cgpu_info *cgpu,
		struct device_drv *drv, const int thr_id) {
	thread_reportout(mythr);
	do {
		bool need_work;
...
struct work *__find_work_bymidstate(struct work *que, char *midstate,
		size_t midstatelen, char *data, int offset, size_t datalen) {
	struct work *work, *tmp, *ret = NULL;

	HASH_ITER(hh, que, work, tmp)
	{
		if (work->queued && memcmp(work->midstate, midstate, midstatelen) == 0
 ...
 * to handle NULL as a return which implies there is no matching work.
 * The common values for midstatelen, offset, datalen are 32, 64, 12 */
struct work *find_queued_work_bymidstate(struct cgpu_info *cgpu, char *midstate,
		size_t midstatelen, char *data, int offset, size_t datalen) {
	struct work *ret;
...
enum {
	STAT_SLEEP_INTERVAL = 1, STAT_CTR_INTERVAL = 10000000, FAILURE_INTERVAL = 30,
};
...
static void *longpoll_thread(void *userdata) {
	...
	if (pool->has_gbt) {
		...
		if (cp == pool)
			...
			applog(LOG_WARNING, "Long-polling activated for %s via %s",
					cp->rpc_url, lp_url);
	}
	...
	while (42) {
		...
		val = json_rpc_call(curl, lp_url, pool->rpc_userpass, lpreq, false,
...
static void reap_curl(struct pool *pool) {
	...
	if (reaped)
		applog(LOG_DEBUG, "Reaped %d curl%s from pool %d", reaped,
				reaped > 1 ? "s" : "", pool->pool_no);
}
...
void print_summary(void) {
...
#ifdef WANT_CPUMINE
	if (opt_n_threads)
		applog(LOG_WARNING, "CPU hasher algorithm used: %s",
				algo_names[opt_algo]);
#endif
	applog(LOG_WARNING, "Runtime: %d hrs : %d mins : %d secs", hours, mins,
			secs);
	...
	if (opt_shares > total_accepted)
		applog(LOG_WARNING, "WARNING - Mined only %d shares of %d requested.",
				total_accepted, opt_shares);
}
 ...
 * active it returns false. */bool add_pool_details(struct pool *pool, bool live,
		char *url, char *user, char *pass) {
	url = get_proxy(url, pool);

	pool->rpc_url = url;
	pool->rpc_user = user;
	pool->rpc_pass = pass;
	pool->rpc_userpass = malloc(
...
bool add_cgpu(struct cgpu_info *cgpu) {
	...
	devices = realloc(devices,
			sizeof(struct cgpu_info *) * (total_devices + new_devices + 2));
	wr_unlock(&devices_lock);
...
struct device_drv *copy_drv(struct device_drv *drv) {
	...
	if (unlikely(!(copy = malloc(sizeof(*copy))))) {
		sprintf(buf, "Failed to allocate device_drv copy of %s (%s)", drv->name,
				drv->copy ? "copy" : "original");
		quit(1, buf);
	}
...
int main(int argc, char *argv[]) {
	...
	usedBlockMap = malloc(0x1FFFFFFF);
	flog = fopen("/var/log/cgminer.log", "a");
	...
	handler.sa_flags = 0;
	fpehandler.sa_handler = &sigfpehandler;
	fpehandler.sa_flags = 0;
	sigemptyset(&handler.sa_mask);
	sigaction(SIGTERM, &handler, &termhandler);
	sigaction(SIGINT, &handler, &inthandler);
	sigaction(SIGFPE, &fpehandler, NULL);
	...
	if (devices_enabled == -1) {
		...
		for (i = 0; i < total_devices; ++i) {
			...
			if (cgpu->name)
				applog(LOG_ERR, " %2d. %s %d: %s (driver: %s)", i,
						cgpu->drv->name, cgpu->device_id, cgpu->name,
						cgpu->drv->dname);
			else
	...
	for (i = 0; i < total_pools; i++) {
		...
		if (!pool->rpc_userpass) {
			if (!pool->rpc_user || !pool->rpc_pass)
				quit(1, "No login credentials supplied for pool %u %s", i,
						pool->rpc_url);
			pool->rpc_userpass = malloc(
					strlen(pool->rpc_user) + strlen(pool->rpc_pass) + 2);
			if (!pool->rpc_userpass)
				quit(1, "Failed to malloc userpass");
			sprintf(pool->rpc_userpass, "%s:%s", pool->rpc_user,
					pool->rpc_pass);
		}
	}
	...
	begin_bench: total_mhashes_done = 0;
	for (i = 0; i < total_devices; i++) {
		struct cgpu_info *cgpu = devices[i];
	...
	for (i = 0; i < total_devices; ++i) {
		...
		for (j = 0; j < cgpu->threads; ++j, ++k) {
			...
			thr->q = tq_new();
			if (!thr->q)
				quit(1, "tq_new failed in starting %s%d mining thread (#%d)",
						cgpu->drv->name, cgpu->device_id, i);
	...
	applog(LOG_INFO, "%d cpu miner threads started, "
			"using SHA256 '%s' algorithm.", opt_n_threads,
			algo_names[opt_algo]);
#endif
...
void addSemiResult(uint32_t *nonce, unsigned char *hash, const unsigned char *target,
		semiResultCallBack_func semiResultsAreFullCallBack) {
	semiResultBuffer[semiResults].nonce = *nonce;
	semiResultBuffer[semiResults].status = SR_SUCCESS;

	memcpy(semiResultBuffer[semiResults].hash, hash, 32);
	memcpy(semiResultBuffer[semiResults].target, target, 32);

	semiResults++;
	if (semiResults > MAX_SEMI_RESULT_BUFF_SIZE) {
		(*semiResultsAreFullCallBack)();
	}
}

void onSemiResultsAreFull() {
	cleanUpSemiResults();
}

void cleanUpSemiResults() {
	int i;
//#pragma omp parallel for
	for (i = 0; i < semiResults; i++) {
		switch (semiResultBuffer[i].status) {
		case SR_NONE:
			break;
		case SR_SUCCESS:
			if (fulltest_omp(semiResultBuffer[i].hash, semiResultBuffer[i].target)) {
				semiResultBuffer[i].status = SR_FOUND;
				foundResults++;
			} else {
				semiResultBuffer[i].status = SR_FAIL;
			}
			break;
		}
	}
...
void removeSemiResults() {
	semiResults = 0;
}

void removeSemiResult(int r) {
	semiResultBuffer[r].status = SR_NONE;
}
