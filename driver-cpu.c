 ...
 *
 * Notice:
 *  - added support of superdrive
 *  - modified maximum work to max nonce in case algo
 */
...
extern bool scanhash_supradrive(struct thr_info*, const unsigned char *pmidstate, unsigned char *pdata,
	unsigned char *phash1, unsigned char *phash,
	const unsigned char *ptarget,
	uint32_t max_nonce, uint32_t *last_nonce,
	uint32_t nonce);
...
#endif
    [ALGO_DEVTECH_SUPRADRIVE] = "supradrive",
#ifdef WANT_SCRYPT
    [ALGO_SCRYPT] = "scrypt",
#endif
...
#endif
    [ALGO_DEVTECH_SUPRADRIVE] = (sha256_func) scanhash_supradrive,
#ifdef WANT_VIA_PADLOCK
...
static const uint64_t max_nonce_depended[] = {
	[ALGO_C]		= 0xFFFF,
#ifdef WANT_SSE2_4WAY
	[ALGO_4WAY]		= 0xFFFF,
#endif
#ifdef WANT_ALTIVEC_4WAY
    [ALGO_ALTIVEC_4WAY] = 0xFFFF,
#endif
    [ALGO_DEVTECH_SUPRADRIVE] = (1<<22),
#ifdef WANT_VIA_PADLOCK
	[ALGO_VIA]		= 0xFFFF,
#endif
	[ALGO_CRYPTOPP]		=  0xFFFF,
#ifdef WANT_CRYPTOPP_ASM32
	[ALGO_CRYPTOPP_ASM32]	= 0xFFFF,
#endif
#ifdef WANT_X8632_SSE2
	[ALGO_SSE2_32]		= 0xFFFF,
#endif
...
static enum sha256_algos pick_fastest_algo()
{
	...
	bench_algo(&best_rate, &best_algo, ALGO_DEVTECH_SUPRADRIVE);
...
{
	return max_nonce_depended[opt_algo];
}
